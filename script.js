...
const resetBtn = document.getElementById('pwdReset');
...
verifyResetEmail.addEventListener('click', loginResetEmailCheck);
verifyResetEmail.addEventListener('blur', loginResetEmailCheck);

verifyEmailButton.addEventListener('click', (e) => {
    e.preventDefault();

    if (checkEmail.value.length == 0) {
        loginResetEmailCheck();
        return false;
    }
    ...
    if (userDetails.Email == verifyResetEmail.value) {
        ...
        document.querySelector('h1').innerHTML = "Reset Password Now";
        return true;
    }
})

const resetPasswordNow = document.getElementById('resetPasswordBtn');
